...
async def _send_via_adapter(platform, pconfig, chat_id, chunk, *, thread_id=None, media_files=None,
    ...
    ``title``/``priority`` (ntfy X-Title / X-Priority) ride in metadata; only ntfy surfaces
    priority, so it is forwarded only to the ntfy standalone sender."""
    ...
    try:
        result = await sender(pconfig, chat_id, chunk, thread_id=thread_id, media_files=media_files,
                              force_document=force_document, title=title,
...
async def _send_to_platform(platform, pconfig, chat_id, message, thread_id=None, media_files=None, force_document=False, args=None, title=None, priority=None):
    ...
    ``title``/``priority`` (ntfy X-Title / X-Priority) are forwarded to platforms that surface
