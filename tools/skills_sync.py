...
-> SKIP); MISSING-from-disk skills (in manifest but absent from user dir) are RESTORED — only
the curator suppression list marks a removal as deliberate; upstream-REMOVED ones leave the manifest.

The manifest lives at ~/.hermes/.bundled_manifest -- deliberately OUTSIDE skills/, so a profile
clone inherits the tree and not the inventory. Older installs keep it at skills/.bundled_manifest;
that location is read as a fallback and migrated on the next sync."""
...
# Shared by the canonical location and the pre-relocation one inside skills/,
# which is still read (and migrated away from) for installs that predate the move.
...
def _manifest_file() -> Path:
    return _live(MANIFEST_FILE, _MANIFEST_FILE_AT_IMPORT, lambda: _hermes_home() / MANIFEST_FILENAME)
...
def sync_skills(quiet: bool = False) -> dict:
    ...
    for skill_name, skill_src in bundled_skills:
        ...
        if skill_name in external_index:
            ...
            # This used to be read as "the user deleted it" and skipped forever,
            # which made every accidental loss permanent. A skill can go missing
            # while its manifest entry survives for reasons that have nothing to
            # do with intent: an interrupted copy, a partial profile clone, a
            # stray rmtree. The deliberate-removal signal is the curator
            # suppression list (checked above) — absence of a directory is not
            # consent. Re-seed and report it.
            try:
                _copy_dir(skill_src, dest)
                st.restored.append(skill_name)
                st.manifest[skill_name] = bundled_hash
                if not quiet:
