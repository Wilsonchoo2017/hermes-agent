...
def _handle_create(args: dict, **kw) -> str:
    ...
    # A task addressed to a profile that doesn't exist is stored durably and never
    # spawned: dispatch_once drops it into ``skipped_unassigned`` on every tick,
    # forever, with no error anywhere (fleetctl#350).
    try:
    ...
    if not assignee_known:
        return json.dumps({
            "ok": False,
            "error": f"assignee '{assignee}' is not a profile on this machine — the "
    ...
    with _board(args.get("board")) as (kb, conn):
        ...
        wrote_board = kb._normalize_board_slug(args.get("board")) or kb.get_current_board()
        return _ok(task_id=new_tid, board=wrote_board, **landed,
