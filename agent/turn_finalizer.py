...
# substitutes a placeholder (chat_completion_helpers.py). Persisting one would
# hand the next worker a prior-attempt block that says nothing, so the retry
# would look resumed and would not be; the exception variant would also pipe
# raw provider error text into a later prompt. Treat them as the absence of a
...
def _record_kanban_budget_exhausted(
    ...
    ``summary`` is the dying worker's own account of its work, produced by the
    extra toolless call in ``_handle_max_iterations``. Persisting it is the whole
    point of that call: ``build_worker_context`` reads prior attempts' summaries,
    so the retry resumes instead of re-deriving. An empty summary is logged rather
    than stored silently — it means the fleet paid for an API call and got
    nothing, which is worth knowing.
    """
    raw = summary if isinstance(summary, str) else (str(summary) if summary is not None else "")
